...
import requests
import pandas as pd
website_url = requests.get('https://en.wikipedia.org/wiki/List_of_Nobel_Peace_Prize_laureates').text
...
for i in range(len(eachRow)):
    ...
    if len(a) == 5:
        ...
        name = a[1].text.rstrip()
        country = a[2]
        rational = previousEntry[3].text.rstrip()
        allWinners.append([year,name,country,rational])
    elif len(a) == 2:
        #print(i)
